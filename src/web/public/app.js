...
// ====== Export ======
function exportDocs() {
  // Try API first (local server)
  fetch('/api/docs')
    .then(r => r.text())
    .then(text => downloadText(text, `${DATA.projectName || 'project'}-README.md`))
    .catch(() => {
      // Fallback: generate basic docs from data (GitHub Pages)
      const lines = [
        `# ${DATA.projectName || 'Project'}`,
        '',
        `> Generated by CodeLens — AI-Powered Codebase Intelligence`,
        '',
        `## Overview`,
        `- **Files**: ${DATA.totalFiles || 0}`,
        `- **Lines**: ${DATA.totalLines?.toLocaleString() || 0}`,
        `- **Language**: ${DATA.primaryLanguage || 'Unknown'}`,
        `- **Type**: ${DATA.projectType || 'Unknown'}`,
        '',
        `## Languages`,
        ...(DATA.languages || []).map(l => `- ${l.name}: ${l.count} files (${l.percentage}%)`),
        '',
        `## Top Definitions`,
        ...(DATA.topDeclarations || []).slice(0, 20).map(d => `- ${d.type} ${d.name} (${d.file})`),
        '',
        `## Architecture`,
        '```mermaid',
      ...
      downloadText(lines.join('\n'), `${DATA.projectName || 'project'}-README.md`);
    });
}
...
function exportJSON() {
  downloadText(JSON.stringify(DATA, null, 2), `${DATA.projectName || 'project'}-insights.json`);
}
